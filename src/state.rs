...
use crate::{
    geometry::{make_bus, make_cube, make_cyl, make_plane, make_sphere},
...
impl GridState {
    pub fn new(state: ServerStatePtr, system: PowerSystem) -> GridStatePtr {
        ...
        let base_map = system.floor_plan.as_ref().map(|fp| {
            ...
            let image = state_lock.images.new_component(ServerImageState {
                name: Some("Basemap Image".into()),
                source: ServerImageStateSource::new_buffer(view),
            });

            let tex = state_lock.textures.new_component(ServerTextureState {
                name: Some("Basemap Texture".into()),
                image,
                sampler: None,
            });

            let mat = state_lock.materials.new_component(ServerMaterialState {
            ...
            let geom = make_plane(&mut state_lock, transform, mat);

            state_lock.entities.new_component(ServerEntityState {
