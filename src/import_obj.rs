/// Taken from the platter source
use std::{collections::HashMap, mem::take, str::SplitWhitespace};
...
use colabrodo_server::{server_bufferbuilder::*, server_messages::*, server_state::*};

use crate::utility::{transform_n, transform_p};

pub type ObjectList = Vec<(EntityReference, GeometryReference)>;

/// Import a wavefront OBJ file
pub fn import_file<R: std::io::BufRead>(
    mut from: R,
    ...
    loop {
        line.clear();
        let count = from.read_line(&mut line).unwrap_or_default();
        if count == 0 {
            break;
        }
        if line.starts_with('#') {
    ...
    let all_objs = pack_wf_state(wfobj, transform);
    ...
    for sub_obj in all_objs {
        ...
        let material = if let Some(m) = &material_override {
            m.clone()
        } else {
        ...
        let geom_ref = source
            .build_geometry(state, BufferRepresentation::Bytes(bytes.bytes), material)
...
fn get_concave_vertex(indicies: &[u32], vs: &[VertexTexture]) -> [u32; 4] {
    for window in indicies.windows(4) {
        let v = glm::Vec3::from(vs[window[0] as usize].position);
        let v2 = glm::Vec3::from(vs[window[1] as usize].position);
        let v1 = glm::Vec3::from(vs[window[2] as usize].position);
        let v0 = glm::Vec3::from(vs[window[3] as usize].position);
